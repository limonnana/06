...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'dateOfEvent'> & {
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      dateOfEvent: event.dateOfEvent?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      dateOfEvent: restEvent.dateOfEvent ? dayjs(restEvent.dateOfEvent) : undefined,
    };
  }
